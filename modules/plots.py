...
def plot_fold_changes(df, gene_col, treatment_col, output_dir='outputs'):
    """
    ...
    add_significance_markers(ax, df, treatment_col, fold_change_col, summary)
...
def add_significance_markers(ax, df, treatment_col, fold_change_col, summary):
    """
    Add significance markers (* p<0.05, ** p<0.01, *** p<0.001) between groups.
    """
    groups = summary[treatment_col].tolist()
    means = summary['mean'].tolist()
    max_y = max(means) * 1.15

    # Compare each group against Vehicle
    ...
    for i, group in enumerate(groups):
        if group == 'Vehicle':
            continue
        sample = df[df[treatment_col] == group][fold_change_col]
        _, p = stats.ttest_ind(control, sample)
...
def plot_litter_variance(pct_litter, pct_residual, gene_col, output_dir='outputs'):
    """
    Generate a pie chart showing how much variance
    is explained by litter vs residual.
    """
    gene_display = gene_col.replace('_ct', '').upper()
